...
browser window on the same machine) logging a round for the same opponent
showed a stale roster until it re-rendered for some unrelated reason. The
browser's `storage` event fires only in *other* same-origin tabs/windows,
never the one that made the write, so it's exactly the missing cross-tab
signal every other closed panel in this repo already uses (see
...
[Judge Profiles](judge-profiles.md)'s equivalent predicate exactly.
...
already do — the in-progress "Log a scouted round" form draft, the "Compare
vs. opponent" section's built comparison, and the bulk-import textarea are
all left untouched, only the persisted roster/history re-reads.
...
ignored, mirroring the existing `JudgeProfilesPanel` cases).
