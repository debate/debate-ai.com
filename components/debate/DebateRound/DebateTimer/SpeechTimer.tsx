...
export function SpeechTimer({
   ...
   * This is required because raw chunk concatenation across MediaRecorder
   * sessions produces invalid WebM (each session has its own EBML header).
   */
  async function mergeAudioBlobs(existing: Blob, newSegment: Blob): Promise<{ blob: Blob; durationSeconds: number }> {
    ...
    // Render concatenated PCM offline
    ...
    // Re-encode rendered buffer to WebM via MediaRecorder pipe
   ...
   * Each call to this function resets the in-memory chunks since merging
   * now happens at the storage level.
   */
  const saveRecordingToLocalStorage = useCallback((
    ...
    if (existingRaw) {
      ...
      // First save — decode to capture duration
      decodeBlob(newSegmentBlob)
        .then((buf) => persist(newSegmentBlob, buf.duration))
        .catch(() => persist(newSegmentBlob))
    }
   ...
   * Chunks are reset on every start — merging across sessions now happens
   * at the storage level via Web Audio API decode + re-encode.
   */
  const startRecording = useCallback(async () => {
    try {
      ...
      mr.onstop = () => {
        const label = speechLabel ?? currentSpeech.name
        saveRecordingToLocalStorage(recordedChunksRef.current, currentSpeech.name, label)
      ...
      window.dispatchEvent(
        new CustomEvent("debate-recording-started", { detail: { speechName: currentSpeech.name } })
      )
    } catch (err) {
      console.warn("Microphone access denied or unavailable:", err)
    }
  }, [currentSpeech.name, speechLabel, selectedMicDeviceId, saveRecordingToLocalStorage])
  ...
  }, [state.name, isRecordingEnabled, startRecording, stopRecording])
