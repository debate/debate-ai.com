...
export function TimersPanel({
  ...
  /**
   * Compute the full label for the current speech (name + debater name if available).
   */
  const currentSpeechIndex = speechState.resetTimeIndex
  const currentSpeechName = debateStyle.timerSpeeches[currentSpeechIndex]?.name ?? ""
  const currentSpeechLabel = (() => {
    if (!currentRound) return currentSpeechName
    const lower = currentSpeechName.toLowerCase()
    let debater = ""
    if (lower.includes("1a") || lower.includes("aff")) {
      debater = currentRound.debaters.aff[0]?.split("@")[0] ?? ""
    } else if (lower.includes("2a")) {
      debater = currentRound.debaters.aff[1]?.split("@")[0] ?? ""
    } else if (lower.includes("1n") || lower.includes("neg")) {
      debater = currentRound.debaters.neg[0]?.split("@")[0] ?? ""
    } else if (lower.includes("2n")) {
      debater = currentRound.debaters.neg[1]?.split("@")[0] ?? ""
    }
    return debater ? `${currentSpeechName} — ${debater}` : currentSpeechName
  })()
  ...
  return (
    <div className="flex flex-col gap-[var(--padding)]">
      ...
      <div className="flex flex-col items-center gap-2 w-full">
        {/* Recordings playback for current speech */}
        <SpeechRecordingPlayer
          speechName={currentSpeechName}
            ...
            setSpeechState((prev) => {
              ...
              if (nextIndex < debateStyle.timerSpeeches.length) {
                return {
                  ...
                  time: debateStyle.timerSpeeches[nextIndex].time * 60 * 1000,
                  state: { name: "paused" },
                }
              }
