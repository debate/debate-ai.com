...
const ProviderSetupModal = ({
  ...
  // qwksearch-api-client exports no model-listing call; upstream
  // qwksearch-web references a `getProviderModels` that doesn't exist
  // anywhere and fails at runtime the same way. Kept as a typed stub so the
  // "fetch models" button degrades to the same error toast.
  const getProviderModels = async (_opts: {
    provider: string;
    config: Record<string, unknown>;
  }): Promise<{ data?: { models?: any[] }; models?: any[] }> => {
    throw new Error('Listing models for this provider is not supported yet');
  };
      ...
      <DialogContent className="w-full max-w-2xl max-h-[90vh] flex flex-col border bg-light-primary dark:bg-dark-primary border-light-secondary dark:border-dark-secondary p-0" hideCloseButton>
        ...
        <div className="px-6 pt-6 pb-4">
          <div className="flex items-center gap-3">
            <ProviderIcon providerType={provider.key} size={24} />
            <div>
