  ...
  return (
    <DropdownMenuContent side={side} align="end" className="w-48">
      <DropdownMenuItem onSelect={(e) => { e.preventDefault(); router.push("/reason-editor") }}>
        <Image src={IconBook} alt="" width={16} height={16} unoptimized className="mr-2 h-4 w-4" />
        Reason Editor
      </DropdownMenuItem>
      <DropdownMenuItem onSelect={(e) => { e.preventDefault(); router.push("/doc") }}>
        <Image src={IconRead} alt="" width={16} height={16} unoptimized className="mr-2 h-4 w-4" />
        Debate Docs
      </DropdownMenuItem>
      <DropdownMenuSeparator />
      ...
      <Dock direction="middle" className={dockClassName}>
        {allItems.map(({ key, label, icon, active, onClick }) => (
          ...
          >
            ...
            <DockIcon>
              <Image src={icon} alt={label} width={24} height={24} unoptimized className="w-full h-full" />
            </DockIcon>
          </DockItem>
        ))}
        <DropdownMenuTrigger asChild>
          <DockItem className="flex flex-col items-center gap-0.5 rounded-full transition-colors cursor-pointer bg-gray-200 dark:bg-neutral-800">
            ...
            <DockIcon>
              <Image src={IconSettings} alt="settings" width={24} height={24} unoptimized className="w-full h-full" />
            </DockIcon>
          </DockItem>
        </DropdownMenuTrigger>
      </Dock>
