...
export const PUT = withRouteErrors(
  "PUT /api/counsel-panel-assessments/[assessmentId]",
  async (req: NextRequest, { params }: { params: Promise<{ assessmentId: string }> }) => {
...
export const DELETE = withRouteErrors(
  "DELETE /api/counsel-panel-assessments/[assessmentId]",
  async (req: NextRequest, { params }: { params: Promise<{ assessmentId: string }> }) => {
    const { assessmentId } = await params
