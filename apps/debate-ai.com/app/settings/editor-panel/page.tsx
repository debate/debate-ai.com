 ...
 * fully owns — importing it into the app's normal component tree would
 * fight the app's own Tailwind base styles document-wide. Rendering it here
 * and embedding this route in a same-origin `<iframe>`
 * (`EditorPreferencesPanel.tsx`) keeps that stylesheet's global reach
 * confined to this one document, in either direction: the app's styles
 * never bleed in, and CardMirror's never bleed out.
 *
 * Same-origin, so no postMessage/CORS dance is needed for state — this page
 * fetches/saves `/api/settings` directly and shares `localStorage` with the
 * parent page's origin, which is all CardMirror's own settings store needs.
 * It does post a `pmd-settings-panel-height` message so the parent iframe
 * wrapper can size itself to content instead of a fixed height.
 */
...
// this route's own chunk — never loaded by the app's main bundle.
...
// this app, and the whole point of loading them via `import()` inside
// `useEffect` is that they (and CardMirror's global stylesheet) only ever
// load in this isolated iframe document, never in the app's main bundle.
...
export default function EditorSettingsPanelPage() {
  ...
  // stylesheet, hydrate from the signed-in user's saved values, then build
  // every category's panel (cheap — these are plain settings rows, not the
  // full editor) and mount the initially-active one.
  useEffect(() => {
    let cancelled = false
  ...
  useEffect(() => {
    if (!ready || isLoading || !isAuthenticated) return
