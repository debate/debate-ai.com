...
| `options.html` | `entrypoints/options` | Every setting for all three tools, in one page. |
...
entrypoints/
  ...
  options/               one settings page for all three tools
src/
...
- `bun run test` covers the parts that are plain functions over a DOM — the
  ...
  this app is outside the repo's `bun`/`turbo` workspaces, so CI never runs
  even the tests that exist, and the panel, sign-in and the timer are verified
  by loading the built `.output/chrome-mv3` unpacked.
