 ...
 * @returns 
 */
export async function parseDebateDocx(docxPath) {
  ...
  cards = cards
    .map((card, index) => {
      ...
      cardhash = cardhash.substring(0, 32 - textLength?.length) + textLength;
  ...
  var fileDate = docxPath instanceof String ? fs.statSync(docxPath).mtime.toISOString().split("T")[0] : "";
