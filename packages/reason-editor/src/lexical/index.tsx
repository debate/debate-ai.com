...
/**
 * Configuration for customizing the toolbar.
 */
export interface ToolbarCustomization {
  /** IDs of built-in toolbar items to hide. See ToolbarPlugin for IDs. */
  hide?: string[];
  /** Ordered list of toolbar item IDs. Items not listed render after, in default order. */
  order?: string[];
  /** Extra toolbar items appended to the toolbar (after built-ins). */
  children?: ReactNode;
}

/**
 * Props shared by EditorContent and EditorWithToolbar.
 */
export interface EditorProps {
  /** Custom Lexical plugins or other children rendered inside the editor container. */
  children?: ReactNode;
  /** Toolbar customization: hide / reorder built-in items, and append custom items. */
 ...
 *
 * @param props - Optional plugin children and toolbar customization.
 * @returns The rendered editor content.
 */
function EditorContent({ children, toolbar }: EditorProps = {}): JSX.Element {
 ...
 *
 * @param props - Optional plugin children and toolbar customization forwarded to EditorContent.
 * @returns The rendered Lexical wrapper.
 */
function LexicalWrapper({ children, toolbar }: EditorProps = {}): JSX.Element {
  const {
      ...
      <LexicalExtensionComposer extension={app} contentEditable={null}>
        <SharedHistoryContext>
          <AutoHighlightProvider>
            <TableContext>
              <ToolbarContext>
                <EditorContent toolbar={toolbar}>{children}</EditorContent>
              </ToolbarContext>
            </TableContext>
          </AutoHighlightProvider>
        </SharedHistoryContext>
 ...
 * Provides all the necessary context providers (Settings, FlashMessage) and wraps
 * the Lexical editor shell. Accepts custom plugins as children and a toolbar config
 * for hiding, reordering, and appending toolbar items.
 *
 * @param props.children - Custom Lexical plugins / React nodes mounted inside the editor.
 * @param props.toolbar - Toolbar customization: hide IDs, order IDs, and extra items.
 * @returns The full Lexical editor with toolbar and contexts.
 */
function EditorWithToolbar({ children, toolbar }: EditorProps = {}): JSX.Element {
  return (
    <SettingsContext>
      <FlashMessageContext>
        <LexicalWrapper toolbar={toolbar}>{children}</LexicalWrapper>
      </FlashMessageContext>
    </SettingsContext>
  );
}
