...
/**
 * IDs of the built-in toolbar items, in their default render order.
 * Pass any of these to ToolbarPlugin's `hide` or `order` prop to customize the toolbar.
 */
export const TOOLBAR_ITEM_IDS = [
  ...
  'alignment',
  'insert',
  'divider-after-insert',
  'share',
  'clipboard',
  'custom',
] as const;

export type ToolbarItemId = (typeof TOOLBAR_ITEM_IDS)[number] | string;

/**
 * The main Toolbar component that provides editor controls.
 * @param props.editor - The root editor instance.
 * @param props.activeEditor - The currently focused editor instance (may be nested).
 * @param props.setActiveEditor - Updates the active editor.
 * @param props.setIsLinkEditMode - Toggles link editing mode.
 * @param props.children - Extra toolbar items appended after built-ins (rendered under id "custom").
 * @param props.hide - Item IDs to hide. See TOOLBAR_ITEM_IDS for the built-in IDs.
 * @param props.order - Ordered list of item IDs. Listed items render in this order; unlisted items render after, in default order.
...
export default function ToolbarPlugin({
  ...
  const items: Array<{ id: ToolbarItemId; render: () => JSX.Element | null }> = [
    {
      id: 'undo',
      render: () =>
    ...
    {
      id: 'divider-after-history',
      render: () => <Divider key="divider-after-history" />,
    },
    {
      id: 'blockFormat',
      render: () =>
    ...
    },
    {
      id: 'insert',
      render: () =>
        toolbarState.blockType !== 'code' && canViewerSeeInsertDropdown ? (
          <DropDown
            key="insert"
            disabled={!isEditable}
            buttonClassName="toolbar-item spaced"
    ...
    {
      id: 'divider-after-insert',
      render: () =>
        toolbarState.blockType !== 'code' && canViewerSeeInsertDropdown ? (
          <Divider key="divider-after-insert" />
        ) : null,
    },
    {
      id: 'share',
      render: () =>
        toolbarState.blockType !== 'code' && canViewerSeeInsertDropdown && documentId ? (
          <div key="share" className="toolbar-item spaced">
            <InviteDropdown
    ...
    {
      id: 'custom',
      render: () => (children ? <Fragment key="custom">{children}</Fragment> : null),
    },
  ];
