 ...
 * call) rather than standing up a second route — via `debate-api-client`'s
...
/**
 * Requests an AI judge decision for `input` from `/api/reason-ai`, returning
 * the parsed `JudgeDecisionAiResult`.
 *
 * Throws a plain `Error` with a useful message when the request fails or
 * when the response text doesn't parse into a valid decision.
 */
export async function requestJudgeDecision(
  ...
  if (error) {
    throw new Error("AI judge decision request failed.");
  }

  const result = parseJudgeDecisionAiResponse(data?.text ?? "");
  if (!result) {
