 ...
 * #17, first slice). Kept separate from `state/userSettings.ts`'s pure
 * validation/merge helpers so those stay unit-testable without mocking
 * `fetch`, mirroring `round/judge-decision-client.ts`'s split.
 *
 ...
 * `settings` singleton for signed-out users instead of showing an error.
 *
