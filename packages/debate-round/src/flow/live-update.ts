 ...
 * persisted note list. A `null` key (e.g. from `localStorage.clear()`)
 * counts too, for the same reason as `isFlowLiveUpdateStorageEvent` above.
 */
export function isPrepNotesPanelLiveUpdateStorageEvent(event: { key: string | null }): boolean {
  return (
    event.key === null ||
 ...
 * `debate-data-sync`'s `state/opponentTeamProfiles.ts`'s own
 ...
 * rounds" list), plus that same module's `"opponentRoundRecordEditHistory"`/
 * `"opponentRoundRecordRedoHistory"` (which rounds show an Undo/Redo action)
 * — mirrors `debate-speech-writer`'s `JUDGE_PROFILES_LIVE_UPDATE_STORAGE_KEYS`
 * exactly, closing the same "every other localStorage-backed panel in this
 * repo still has no cross-tab live-update mechanism" Known gap noted in
 * `shared-flow-sync.md`, for this panel's own stores.
 */
