 ...
 * (see `debate-data-sync`'s `state/opponentTeamProfiles.ts` and
 * `state/opponentRoundRecords.ts`): `opponentTeamProfiles` (the aggregated
 ...
 * decide whether a logged round shows an Undo/Redo action) — mirroring the
 * `JUDGE_PROFILES_LIVE_UPDATE_STORAGE_KEYS` split in `debate-speech-writer`'s
 * `state/live-update.ts` exactly.
 */
 ...
 * re-read its persisted roster and logged-round history. A `null` key (e.g.
 * from `localStorage.clear()`) counts too, for the same reason as
 * `isFlowLiveUpdateStorageEvent` above.
 */
