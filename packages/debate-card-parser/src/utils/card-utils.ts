...
import { extractMarked } from "../extractors/text-extractor";
import type { Card, FormatProfile, MutableCard } from "../types/types";
...
export const getBlueShade = (count: number) => {
  ...
  if (count >= 100)
    return "bg-blue-300 text-blue-900 dark:bg-blue-300 dark:text-blue-900";
  if (count >= 50)
    return "bg-blue-200 text-blue-800 dark:bg-blue-200 dark:text-blue-800";
...
export const getGreenShade = (wordCount: number) => {
  ...
  if (wordCount >= 1000)
    return "bg-green-300 text-green-900 dark:bg-green-300 dark:text-green-900";
  if (wordCount >= 500)
    return "bg-green-200 text-green-800 dark:bg-green-200 dark:text-green-800";
...
export function repairCards(cards: Card[], _profile: FormatProfile): Card[] {
  ...
  for (let i = 0; i < cards.length; i++) {
    ...
    if ((card.words ?? 0) < 20 && i > 0 && !card.cite && !card.author) {
      const prevCard = repaired[repaired.length - 1] as
        | (Card & { body?: string[] })
        | undefined;
