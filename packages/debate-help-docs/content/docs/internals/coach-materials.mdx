...
- `state/coachMaterialVersions.ts` — a store mirroring
  `state/coachMaterials.ts`'s own persistence convention; both now sync to
  the signed-in user's account as the `coachMaterials`/`coachMaterialVersions`
  collections (see [Tool Data Sync](tool-data-sync.md)). Each entry is a
