...
- **Package:** [`debate-round`](https://github.com/debate/debate-ai.com/blob/master/packages/debate-round/README.md)
...
- **Store:** `state/prepNoteReplies.ts` (localStorage: `prepNoteReplies`),
  ...
  thread oldest first.
- **Like `dailyBestCardComments.ts`,** this store syncs to the signed-in
  user's account as the `prepNoteReplies` collection, the same generic sync
  `prepNotes` itself rides on (see Known gaps below and
  [Tool Data Sync](tool-data-sync.md)).
- **UI:** `panels/PrepNotesPanel.tsx`'s `PrepNoteReplyThread` renders the
  ...
  `PrepNotesPanel` also takes an optional `signedInContributorId` prop
  (mirroring `DailyBestCardPanel`'s identical convention) that prefills a
  reply draft's "Your name" field — `apps/debate-ai.com/components/research/PrepNotesWithIdentity.tsx`
  resolves it from the real signed-in session via
  `deriveContributorIdFromSessionIdentity`. A starting value only; typing
  over it is always respected.
...
- ~~The new note-creation popover's "Author ID" is a free-form typed field,
  ...
  id).~~ Closed: `PrepNotesPanel` and `FlowSummariesPanel` (the "Send to
  Prep Notes" form) both take an optional `signedInContributorId` prop —
  the same convention `PrepRoomPanel`/`DailyBestCardPanel` already use —
  and `PrepNotesWithIdentity.tsx`/`FlowSummariesPanelWithPrepNotes.tsx`
  prefill it from the real signed-in session via
  `deriveContributorIdFromSessionIdentity`. Still a prefill, not a login:
  the field stays free-form and typing over it is always respected.
