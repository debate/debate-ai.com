...
- No follow-ups remain open on the "applying a preset doesn't select or
  scroll to a particular round" gap: applying a preset from the panel's
  global list now also scrolls that round's card into view, when the
  preset's origin round still exists (`debate-practice-drills`'
  `state/outlineFilterPresetJump.ts#resolvePresetJumpRoundId`, wired into
  `ArgumentTreePanel`). A preset saved before round-scoped tracking existed,
  or whose origin round's outline has since been cleared, has nothing to
  jump to and just applies the filter as before.
