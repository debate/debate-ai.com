...
- ~~There is no way to download a topic starter's `.cmir`. The file is stored
  ...
  come back out through the editor.~~ **Partially fixed:** `FileTree`'s row
  menu has a "Download as .docx" item for an owned (non-folder) document —
  `ReasonDocsProvider.downloadDocument` runs the row's HTML (whatever shape it
  is stored in — `documentHtml` already resolves that, live edits included)
  through `htmlToDocxBytes` (`lib/cardmirror/stored-cmir.ts`, `htmlToDoc` +
  `debate-editor/engine`'s `toDocx`) and triggers a browser download named by
  `docxDownloadFilename`. `TopicStarterTree` still has no row menu and the
  editor itself exposes no export action, so a topic starter still can't be
  gotten out as a file at all — tracked as a follow-up.
