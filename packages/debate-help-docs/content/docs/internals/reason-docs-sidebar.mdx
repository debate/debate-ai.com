---
title: "REASON docs sidebar — files on the document routes, opened in CardMirror"
---
...
## What else these sidebars carry
...
`showsResearchOnlySidebar` (same module) marks the `/cards` and
`/reason-editor` subtrees, and there `AppSidebarShell` renders the tree as
`ToolNavTree sectionIds={[…]}`: only the named sections, no "Apps" node above
them and no reference pair below, and no footer.
...
and the editor alongside them, so it is the one kept — and because `/cards`
matches the *Apps* node by route, `ToolNavTree` falls back to opening the
first section it was given rather than leaving a heading with nothing under
it. The dock stays: it is the control the reader clicked "Shared" in, and the
way back to `/videos`.

`/reason-editor` is trimmed for the same reason `/cards` is, not because of
where it is reached from: its sidebar is the file tree, and Coaching and
Practice stacked under a file tree are two more navigations on a page about
writing. Research is the section the editor is itself listed in and the one
whose tools it is used alongside.
...
- **Forgiving.** A URL that names only the file (`?doc=warming-1ac`) resolves
  ...
  before paths keep working. A rename is forgiven once: `documents.previous_slug`
  holds the file's own slug from just before its last rename, and a URL that
  matches neither the current path nor any other file's resolves against that
  instead before falling through to the fallback.
...
Above the panels sits the row of actions on files: New file, New folder,
Upload, and — on the editor route, signed in, with a document open — **Share
with contacts** (`components/reason-editor/ShareWithContacts`, drawn in its
`icon` variant so it matches the three beside it). Sharing was a labelled
button in the editor's own header strip; it is an action on the open file, so
it belongs in the row that file's other actions are in. It is scoped to the
editor route because sharing starts a live co-editing session through the
CardMirror engine, and that page is what mounts the engine — on `/cards` the
control would open a dialog that could only fail.
...
- ~~There is no way to download a topic starter's `.cmir`. The file is stored
  ...
  come back out through the editor.~~ **Fixed:** both `FileTree`'s and
  `TopicStarterTree`'s row menus have a "Download as .docx" item (not offered
  for a folder row — there is nothing to convert). `FileTree`'s wires to
  `ReasonDocsProvider.downloadDocument`, which runs the row's HTML (whatever
  shape it is stored in — `documentHtml` already resolves that, live edits
  included) through `htmlToDocxBytes` (`lib/cardmirror/stored-cmir.ts`,
  `htmlToDoc` + `debate-editor/engine`'s `toDocx`) and triggers a browser
  download named by `docxDownloadFilename`. `TopicStarterTree`'s wires to the
  new `ReasonDocsProvider.downloadTopicDocument`, the same conversion run
  against `topicStarterHtml(item)` instead — a topic starter is read-only, so
  unlike `downloadDocument` there is no live-edit lookup to resolve first.
  Both share the actual browser-download trigger (`triggerDocxDownload`, a
  small helper in `ReasonDocsProvider.tsx`) so the `Blob`/anchor-click
  mechanics live in one place. `ReasonDocsSidebarPanels` shows a download
  failure from either tree under that tree specifically (`downloadError` for
  Files, `topicDownloadError` for Topic Starters), not the other. The editor
  itself still exposes no export action of its own — opening the sidebar row
  menu is still the only way to export — which is unchanged and not tracked
  as a gap, since it was never a stated goal. Vitest-covered:
  `apps/debate-ai.com/lib/cardmirror/__tests__/stored-cmir-render.test.ts`
  adds the `storedContentToHtml`/`topicStarterHtml` coverage this download
  path leans on (a legacy HTML row passed through untouched, a `.cmir` row
  parsed and reserialized, format-less sniffing, an absent-content row, and a
  `.cmir` that fails to parse rendering its escaped-safe notice instead of
  throwing) that neither function had before — previously they only fed the
  editor's `content` prop, where the same bug would have been a rendering
  glitch rather than a broken or silently wrong file download.
- ~~Renaming a file changes its URL, and nothing redirects the old one — it
  ...
  resolves.~~ **Fixed:** `documents` gets a nullable `previous_slug` column
  (`drizzle/0046_document_previous_slug.sql`). `PUT /api/doc/documents/:id`
  sets it to the file's own old slug (`slugifySegment(doc.title)`) whenever a
  title edit actually changes that slug — a capitalization or punctuation-only
  edit that resolves to the same slug leaves it alone, and an old title with
  nothing sluggable in it has no slug worth remembering. `findItemByRef`
  (`lib/reason-docs/doc-path.ts`) tries each file's *previous* path — its
  current path with only the leaf segment swapped back to `previousSlug` — the
  same exact-then-suffix way it already tries the current path, after both of
  those come up empty and before falling through to the folder/first-file
  fallback. This remembers one rename back, not a full history: a link from
  before a file's second-most-recent rename still falls through, and a folder
  rename (which moves every file under it) isn't tracked here at all — only a
  file's own leaf-level rename is.
