...
`useVideoState` and is sent to the server. Two preferences are applied on the
client because the feed query has no knowledge of them: **favourites** are sent
as an explicit `ids` allow-list when the favourites filter is on, and **hidden
