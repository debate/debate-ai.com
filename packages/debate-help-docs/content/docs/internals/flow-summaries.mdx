...
The "Your name" form also takes an optional `signedInContributorId` prop
(mirroring `PrepRoomPanel`'s identical convention) that prefills it from a
real signed-in session — the wrapper resolves this via
`deriveContributorIdFromSessionIdentity`, the same as
`PrepNotesWithIdentity.tsx` does for `PrepNotesPanel`'s own reply field.
Still a starting value only; typing over it is always respected. Closes
`prep-notes.mdx`'s Known gaps entry noting this field had no link to a real
identity.
