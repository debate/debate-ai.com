...
- ~~There is no way to download a topic starter's `.cmir`. The file is stored
  ...
  **Partially fixed:** the Files panel's row menu now has a "Download as
  .docx" item for your own documents. Topic starters don't have this yet —
  there is still no way to get one out as a file at all, from the sidebar or
  the editor.
