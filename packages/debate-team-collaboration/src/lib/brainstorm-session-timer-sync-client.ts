/**
 * @fileoverview Network calls for the Team Brainstorm Assist session-timer
 * account sync (see `brainstorm-session-timer-sync.ts`). Talks directly to
...
/**
 * Fetches the current user's synced session timer. Returns `null` when
 ...
 * synced yet resolves to `{ timer: null }`, distinct from the signed-out
 * case.
 */
...
/**
 * Saves (or, with `null`, clears) the synced session timer for the current
 * user. Throws (with the server's `{ error }` message when present) on a
...
export async function saveBrainstormSessionTimer(
  state: BrainstormSessionTimerSyncPayload | null,
  endpoint = "/api/settings",
): Promise<void> {
  const res = await fetch(endpoint, {
    method: "PUT",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ brainstormSessionTimer: state }),
  });
  if (!res.ok) {
