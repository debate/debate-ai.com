...
/**
 * @fileoverview The settings UI shared by two hosts: `SettingsPanel` wraps
 * this in a full-screen `Dialog` for the in-app "Settings" shortcut/menu
 * item, and `EmbeddedSettingsPanel` mounts it directly (no dialog chrome) so
 * a page like the app-wide `/settings` page can show ebb's preferences
 * inline alongside every other app's. All the settings state lives here
 * (rather than in the dialog wrapper) since Base UI's `Dialog.Popup`
 * unmounts its children on close, which already resets this component's
 * transient UI state (the open category, in-progress shortcut recording,
 * the shortcut filter) for the next open — no separate reset-on-close effect
 * needed.
 */
...
export default function SettingsPanelBody() {
    ...
    // Mirror the stored default zoom into the editable field, and on
    ...
    return (
        <div className="flex min-h-0 flex-1" onKeyDown={onPanelKeyDown}>
            {/* Left nav */}
            <nav
