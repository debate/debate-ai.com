...
- **Coaching Programs and Group Challenges — roundId-to-contributor mapping
  ...
  in the Product Feature Ideas list: "a `roundId`-to-contributor mapping so
  a member's already-flowed practice round can generate a drill set on this
  board" — also the sole "Known gap" `docs/features/coaching-programs.md`
  named. `debate-round`'s new `state/memberPracticeFlows.ts` persists only
  the missing link (which `roundId`, and which side, belongs to which
  contributor) rather than inventing a new copy of a round's flowed
  content: its `resolveFlowForRound`/`buildMemberPracticeFlowsForRoster`
  resolve a contributor's registered round directly from the live round
  editor's own `rounds`/`flows` `localStorage` keys — the same storage
  `dialogs/CreateRoundDialog/useRoundEditorForm.ts` already writes and
  `hooks/useRoundFromSlug.ts` already reads (`round.flowIds.includes(f.id)`)
  — picking the round's flow with the most content when it has more than
  one. `state/persistedCoachingProgramBoard.ts`'s
  `buildPersistedCoachingProgramBoard` now composes `memberFlows` from this
  store whenever a caller doesn't supply one explicitly (an explicit list,
  including `[]`, still overrides it), so a roster member with a
  registered, resolvable practice round gets a real, non-empty drill set on
  their program's board instead of always rendering empty. A registered
  round that can't be resolved (not flowed yet, or missing) is skipped from
  `memberDrills` rather than rendered broken. `CoachingProgramsPanel`'s
  board view now has a "Member practice flows" section per roster member —
  a round id + side registration form, a resolution-status badge ("drills
  ready" / "not flowed yet" / "no practice round registered"), and a
  "Clear" action — wired to `saveMemberPracticeFlow`/
  `deleteMemberPracticeFlow`. Vitest-covered in
  `packages/debate-round/test/memberPracticeFlows.test.ts` (CRUD against
  empty/corrupt/non-array storage; `resolveFlowForRound` against a missing
  round, a round with no matching flow, a single matching flow, and picking
  the larger of two matching flows; `buildMemberPracticeFlowsForRoster`
  resolving a real drill-ready flow, skipping a contributor outside the
  roster, skipping an unresolvable registration, and sorting by
  contributor) and new cases in
  `packages/debate-round/test/persistedCoachingProgramBoard.test.ts`
  (resolving a registered, flowed round into a real, non-empty drill set,
  and an explicit `memberFlows` argument overriding the persisted
  composition). Documented at `docs/features/coaching-programs.md` (data
  flow + "Member practice flows" section updated, "Known gaps" replaced).
  Verified: `bun run test` (149 files / 2033 tests passed), `bun run
  typecheck` (11 packages passed), `bun run build:web` (built successfully,
  `/coaching-programs` in the built route list) — no repo-wide `lint`
  script exists.
