...
### Completed
- **Coaching Programs and Group Challenges — roundId-to-contributor mapping
  for member drills.** Closes the remaining half of the "(b-continued)"
  follow-up named under idea #13 ("Coaching Programs and Group Challenges")
  in the Product Feature Ideas list: "a `roundId`-to-contributor mapping so a
  member's already-flowed practice round can generate a drill set on this
  board — still not started." `debate-round`'s new
  `state/memberRoundAssignments.ts` persists a `{ programId, contributorId,
  roundId, sideKey }` mapping to localStorage, scoped per
  `(programId, contributorId)` pair so the same member can be assigned a
  different round in a different coaching program. Its new
  `state/liveFlows.ts` resolves that `roundId` — the same
  `String(flow.id)` convention `hooks/useWordCountSpeechMode.ts` already
  uses for the live flow editor's own round identity, not this package's
  separate "practice round" session ids (`aiVersusRounds.ts`/
  `practiceRounds.ts`) — back to a real, already-flowed `Flow`'s
  `children`/`columns` by reading the live editor's existing flat `flows`
  localStorage array (`hooks/useFlowEffects.ts`). `state/persistedCoachingProgramBoard.ts`'s
  new `buildMemberFlowsFromAssignments` composes a program's stored
  assignments against those resolved flows into `buildCoachingProgramBoard`'s
  `memberFlows` input (skipping an assignment whose `roundId` doesn't resolve
  to a stored flow rather than throwing), and `buildPersistedCoachingProgramBoard`
  now defaults to that composition whenever a caller doesn't supply its own
  `memberFlows` — mirroring `topicSprints.ts`'s "compose every input from its
  own store, but let a caller override" convention. `CoachingProgramsPanel`
  gains a "Member round assignments" form (round id + side per roster member,
  with Save/Clear actions) once a program's board is open, and the board now
  renders each member's resolved drill set via `buildMemberDrillSummaryText`
  alongside the existing summary block — closing idea #13's "(b-continued)"
  follow-up in full; no follow-ups remain open on this idea except (c),
  Practice Round Simulator wiring into the coaching space, which stays open
  as previously noted. Vitest-covered in
  `packages/debate-round/test/memberRoundAssignments.test.ts` (empty/corrupt/
  non-array storage, save/lookup, per-`(programId, contributorId)` scoping
  and overwrite, delete), `packages/debate-round/test/liveFlows.test.ts`
  (empty/corrupt/non-array storage, resolving a stored flow by its
  stringified id, an unmatched id returning `undefined`), and extended
  `packages/debate-round/test/persistedCoachingProgramBoard.test.ts`
  (`buildMemberFlowsFromAssignments` resolving/skipping assignments; a
  board composing a real drill set from a saved assignment + live flow; an
  unresolved `roundId` leaving `memberDrills` empty; an explicit `memberFlows`
  argument overriding the stored composition). Docs updated at
  `docs/features/coaching-programs.md`. Verified: `bun install`, `bun run
