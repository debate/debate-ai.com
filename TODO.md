...
### Completed
- **Progress Unlocks — feed completed-task history into tier computation.**
  Closes follow-up (c) under the "📈 Research Progress Tracking" bullet,
  "feeding a contributor's topic-progress history back into
  `progress-unlocks.ts`'s tier computation" (the same gap named from the
  other side under "🔓 Progress Unlocks"). A new
  `packages/debate-card-search/src/lib/unlock-progress-status.ts` adds
  `computeContributorTierWithProgress`/`buildContributorUnlockStatusWithProgress`/
  `buildUnlockStatusWithProgressText`, mirroring `lib/progress-unlocks.ts`'s
  own tier-search shape but gating each tier with an additional
  `minCompletedTaskCount` threshold (`DEFAULT_PROGRESS_AWARE_UNLOCK_TIER_REQUIREMENTS`:
  5 completed tasks for `veteran`, 15 for `expert`, 0 for `novice`/`apprentice`
  so early progression is unaffected) — `progress-unlocks.ts` itself is
  reused unchanged (`getUnlockedSkillLevel`/`getUnlockedBadges`), not
  duplicated. `state/researchProgress.ts` gained
  `getPersistedContributorProgress`, building one contributor's
  `ContributorProgress` (in particular their `totalCompletedTasks`) directly
  from the persisted completed/active-assignment stores rather than building
  the whole `buildPersistedResearchProgressBoard` and searching it.
  `lib/unlock-streak-status.ts`'s `buildContributorUnlockStatusWithStreak`/
  `buildContributorUnlockStatusWithStreakFromStore` now derive tier through
  this task-gated computation instead of `progress-unlocks.ts`'s plain,
  task-blind one — `completedTaskCount` is a new optional parameter
  (defaulting to 0) on the pure function, so no existing caller's behavior
  changes unless it opts in. `panels/ProgressUnlocksPanel.tsx` (`/cards/progress`)
  gained a "Tasks done" column and its next-tier text now also names how
  many more tasks are needed. Vitest-covered in
  `packages/debate-card-search/test/unlock-progress-status.test.ts` (tier
  computation, status building, text rendering — including caller-supplied
  requirement tables and the top-tier no-next-tier case),
  `packages/debate-card-search/test/unlock-streak-status.test.ts` (updated to
  cover the task-completion gate holding a contributor at a lower tier until
  they finish enough tasks, both against caller-supplied stats and against
  real persisted routed/completed tasks), and
  `packages/debate-card-search/test/researchProgress.test.ts` (the new
  `getPersistedContributorProgress`: zeroed-out result for an unknown
  contributor, matching the full board's entry for a known one, and
  isolation between two contributors' completed/active tasks). Documented in
  `docs/features/progress-unlocks.md` and `docs/features/research-progress-tracking.md`.
  No repo-wide `lint` script exists (checked root/app/package `package.json`
  scripts) so none was run. Verified: `bun install` (2050 packages), `bun run
  test` (98 files / 1393 tests, all pass), `bun run typecheck` (11 of 12
  in-scope packages have a typecheck script; all pass), and `bun run
  build:web` (`debate-ai-web`, succeeds, `/cards/progress` route present)
  all pass.
