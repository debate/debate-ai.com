...
- **🕵️ Opponent Team Profiles — cross-tab live update.** Another repeat of
  ...
  prior two runs' own completed-task notes, confirmed still missing a
  `storage`-event listener by grepping every panel in the repo.

  Extended `packages/debate-round/src/flow/live-update.ts` (which already
  held `PreRoundBriefingsPanel`'s and four other panels' own predicates)
  with `OPPONENT_TEAM_PROFILES_PANEL_LIVE_UPDATE_STORAGE_KEYS`/
  `isOpponentTeamProfilesPanelLiveUpdateStorageEvent`, covering all five of
  the panel's backing stores: `opponentTeamProfiles` (the scouting roster
  table), `opponentRoundRecords` (the logged-rounds list a profile is
  re-aggregated from), `opponentRoundRecordEditHistory`/
  `opponentRoundRecordRedoHistory` (which decide whether a logged round
  shows an Undo/Redo action), and `ownRoundHistory` (this team's own round
  ...
  form draft and any built comparison are left untouched, matching every
  ...
  update" section (plus its Known gaps closure bullet) and
  ...
  staying ignored, mirroring the existing sibling-predicate cases in the
  same file). `UserSettingsPanel` (`debate-round` — its `form` is a live,
  ...
  unsaved in-progress edit), `CoachingProgramsPanel`
