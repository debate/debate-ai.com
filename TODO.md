...
- **Peer Review System — review-queue/comment-thread UI.**
  `packages/debate-card-search/src/panels/PeerReviewsPanel.tsx` adds a
  full-page React panel that reads every persisted `CardReview` via
  `state/peerReviews.ts`'s new `buildPeerReviewsPanelView` (status-grouped,
  reviews needing teammate action — `in_review`/`changes_requested`
  — surfaced first) and renders a review queue: each review's comment
  thread (severity badge, resolve action) plus whichever lifecycle action
  is legal from its current status (submit/resubmit for review, approve,
  request changes, reject, publish), and a minimal "start a review" form
  (card id only) since no card-submission flow exists yet to seed a review
  automatically. Every action calls one of seven new
  `state/peerReviews.ts` functions —
  `submitPersistedReviewForReview`/`requestPersistedReviewChanges`/
  `approvePersistedReview`/`rejectPersistedReview`/`publishPersistedReview`/
  `addPersistedReviewComment`/`resolvePersistedReviewComment` — each a thin
  `applyPersistedReviewUpdate` wrapper that applies the matching pure
  `lib/peer-review.ts` state transition to a stored review and saves the
  result, introducing no new state-machine logic. `approvePersistedReview`
  can throw `UnresolvedBlockingCommentsError` (from `lib/peer-review.ts`)
  before saving; the panel catches it and shows the message inline on that
  review instead of crashing. It's mounted at `/cards/reviews`
  (`apps/debate-ai.com/app/cards/reviews/page.tsx`, with a back-link to
  `/cards`, following the same panel convention as `/cards/revisions`/
  `/cards/inbox`) and reachable from the global nav dock's Settings menu
  ("Peer Review", via a new `MessageSquareCheck`-icon `DropdownMenuItem` in
  `CategoryDock.tsx`). Closes follow-up (a), "a review-queue/comment-thread
  UI that reads/writes through this store," named under the "🗣️ Peer
  Review System" bullet in Research Crowdsourcing Organizer Features
  below — this is the thirteenth "wire a persisted slice's UI into the
  actual web app" follow-up closed in this repo. Vitest-covered in
  `packages/debate-card-search/test/peerReviews.test.ts` (each transition
  function's happy path, that an illegal transition or
  `UnresolvedBlockingCommentsError` throws without saving, and
  `buildPeerReviewsPanelView`'s empty/grouped/ordered output). Follow-ups
  (b) reviewer identity/permission checks once auth/roles exist and (c)
  wiring a review's lifecycle to whatever eventually persists submitted
  cards remain open — neither is started.
  PR: TBD.
...
* 🗣️ Peer Review System - Allow teammates to review, comment on, and refine submitted cards before they go live. _Status: first slices done (see Tracker Status above) — `debate-card-search` now has a `CardReview` status state machine (`createCardReview`/`submitForReview`/`requestChanges`/`approveReview`/`rejectReview`/`publishReview`) plus a blocking-aware comment thread (`addReviewComment`/`resolveReviewComment`/`getUnresolvedBlockingComments`/`isReadyToPublish`/`buildReviewSummary`) that blocks approval until every blocking comment is resolved. A second slice, `peerReviews.ts` (see Tracker Status above), now persists `CardReview` records (including their `ReviewComment` thread) to localStorage, keyed by `cardId`. A third slice, `PeerReviewsPanel` (see Tracker Status above, "Peer Review System — review-queue/comment-thread UI"), now renders a status-grouped review queue at `/cards/reviews` with submit/approve/request-changes/reject/publish and comment/resolve actions, closing follow-up (a). Follow-ups: (b) reviewer identity/permission checks once auth/roles exist, (c) wiring a review's lifecycle to whatever eventually persists submitted cards, so `publishReview` can gate a card actually going live. Neither is started._
