...
### Completed
- **Peer Review System — reviewer identity/permission checks.**
  Closes follow-up (b) named under the "🗣️ Peer Review System" bullet in the
  Research Crowdsourcing Organizer Features list — "reviewer identity/
  permission checks once auth/roles exist." No auth/roles system exists in
  this repo, so this slice mirrors the free-text-identity convention already
  used elsewhere in this package (`team-collaboration-mode.ts`'s
  `SprintNote.authorId`, the Community-Rated Summaries idea's Reviewer ID
  field for the Endorse action) instead of waiting on one. `lib/peer-review.ts`'s
  `CardReview` now carries an optional `authorId` (set by `createCardReview`)
  and `reviewedBy`. `requestChanges`/`approveReview`/`rejectReview`/
  `publishReview` each now require a `reviewerId` argument, validated by a
  new `assertReviewerAllowed`: empty throws `ReviewerIdRequiredError`, and a
  reviewer id matching the review's own `authorId` throws
  `SelfReviewNotAllowedError` — a review started with no author id (the
  field left blank, or a review that predates it) has nothing to guard
  against and accepts any reviewer id, matching this repo's "works
  standalone, gated further once the gating data exists" convention. The
  reviewer id that successfully completes a gatekeeping action is stamped on
  `CardReview.reviewedBy` and surfaced in `buildReviewSummary`.
  `ReviewQueuePanel` gained an optional Author ID field on the start-review
  form, an authorId badge on each review row, and a Reviewer ID input next
  to the lifecycle action buttons (shown once a review reaches `in_review`/
  `approved`) whose value is passed into the gated transitions; a rejected
  attempt surfaces the thrown error's message through the panel's existing
  error banner. Vitest-covered in
  `packages/debate-card-search/test/peer-review.test.ts` (reviewedBy
  stamping, the empty-reviewer-id and self-review failure paths on every
  gated transition, and that a different reviewer id or a missing author id
  is unaffected) and the pre-existing
