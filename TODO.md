...
### Completed
- **Peer Review System — publish gating for the Contributions Feed.** Closes
  follow-up (c) named under the "🗣️ Peer Review System" bullet in the
  Research Crowdsourcing Organizer Features list: "wiring a review's
  lifecycle to whatever eventually persists submitted cards, so
  `publishReview` can gate a card actually going live." `debate-card-search`'s
  `state/contributions.ts` now has `getContributionPublicationStatus`
  (a contribution's status is `"no_review"` until a `CardReview` exists for
  its id, then mirrors that review's `ReviewStatus`), `sendContributionToReview`
  (idempotently starts a `"draft"` `CardReview` keyed by the contribution's
  own id — a no-op for an unknown contribution or one already under review),
  and `isContributionVisibleInPublicFeed`, mirroring
  `evidenceLibraryEntries.ts`'s `saveEvidenceLibraryEntryRevision` "one
  persisted store composes another persisted store directly" convention
  (here composing `lib/peer-review.ts` + `state/peerReviews.ts`).
  `buildPersistedContributionFeed` now stamps every entry's
  `publicationStatus` and accepts a `{ publicOnly: true }` option that drops
  any entry still stuck in review — a contribution never sent to review
  stays visible, so this doesn't retroactively hide anything that predates
  the Peer Review System. `ContributionsFeedPanel` now renders each entry's
  publication-status badge, a "Send to review" action on any entry not yet
  under review, and a "Public feed only" toggle that demonstrates the gate.
  No follow-ups remain open on the "🗣️ Peer Review System" bullet besides
  (b), reviewer identity/permission checks, which stays blocked on this repo
  having no auth/identity system. Vitest-covered in
  `packages/debate-card-search/test/contributions.test.ts` (status
  derivation for a reviewed/unreviewed/unknown contribution, idempotent
  send-to-review, and public-feed filtering that hides an in-progress
  review while keeping a never-reviewed or published contribution visible).
  Docs updated at `docs/features/review-queue.md` and
  `docs/features/contribution-leaderboard.md`. Verified: `bun run test`
  (146 files / 2001 tests passed), `bun run typecheck` (11 packages
  passed), `bun run build` (both buildable packages passed, `/cards/reviews`
  and `/cards/contributions` both appear in the built route list) — no
