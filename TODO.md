...
### Completed
- **Top Contributor Awards — "original argument" and "refutation" award
  categories.** Closes follow-up (a) named under the "🏆 Top Contributor
  Awards" bullet in the Research Crowdsourcing Organizer Features list: "a
  ...
  today." `debate-card-search`'s `lib/community-rating.ts` adds `"argument"`
  and `"refutation"` to the `ContributionKind` union alongside the existing
  `"card"`/`"summary"`/`"highlight"`/`"annotation"` kinds — every other
  helper in this idea's dependency chain (`community-rating.ts`'s scoring,
  `contribution-leaderboard.ts`'s per-contributor aggregation,
  `daily-quests.ts`'s quest targets) already worked generically off
  `ContributionKind`, so no logic changes were needed there.
  `lib/contributor-awards.ts`'s `DEFAULT_AWARD_CATEGORY_LABELS` gains
  `argument: "Best Original Argument"` and `refutation: "Best Refutation"`
  entries, and `buildTopContributorAwards`'s stable kind order now includes
  both, so a contributor whose attributed contributions are tagged
  `"argument"` or `"refutation"` can now win a category award exactly like
  the original four kinds. The `"kind"` selector in `ContributionsFeedPanel`
  (plus its badge-variant map), `DailyQuestsPanel`, and
  `GroupChallengesPanel` all gained matching "Argument"/"Refutation" options,
  so a contribution, quest target, or group-challenge goal can be tagged
  with either kind from the existing UI — no new panel or route was needed.
  Vitest-covered in `packages/debate-card-search/test/contributor-awards.test.ts`
  (new argument/refutation award-selection, announcement-text, and
  six-kind stable-ordering cases). No follow-ups remain open on this bullet
  beyond the (b)/(c) items already noted (a scheduled announcement job and
  an awards UI — the UI already existed; only the job remains). Docs updated
  at `docs/features/contributor-awards.md`. `bun run test` (1873 tests),
  `bun run typecheck` (11 packages), and `bun run build` all pass; no
  repo-wide `lint` script exists, matching this bullet's prior entries.
...
* 🏆 Top Contributor Awards - Give recognition for best evidence finder, best explainers, best original argument, and best refutations. _Status: first slices done (see Tracker Status above) — `debate-card-search` now has `buildTopContributorAwards`/`buildCategoryLeaderboard`/`groupContributionsByKind`/`buildAwardsAnnouncementText` for grouping contributor-attributed contributions by `ContributionKind` and selecting a per-kind category winner by helpfulness score, reusing the existing idea #11/Contribution Leaderboard scoring. A second slice, `ContributorAwardsPanel` (see Tracker Status above, "Top Contributor Awards — awards UI panel"), now renders every category's current winner at `/cards/awards`, closing follow-up (c). A third slice (see Tracker Status above, "Top Contributor Awards — 'original argument' and 'refutation' award categories") added `"argument"`/`"refutation"` to `ContributionKind` plus matching `DEFAULT_AWARD_CATEGORY_LABELS` entries and kind-order/UI-selector wiring, closing follow-up (a). Follow-up (b), a scheduled job to persist/announce winners, remains open — not started._
