...
- **🕵️ Opponent Team Profiles — cross-tab live update.** Another repeat of
  ...
  for `OpponentTeamProfilesPanel` — explicitly named as still open by the
  previous run's own completed-task note, confirmed still missing a
  `storage`-event listener by grepping every panel named in that Known
  gap's history.
  ...
  the panel's backing stores: `opponentTeamProfiles` (the aggregated roster
  the table renders), `opponentRoundRecords` (the logged-round history), and
  ...
  `debate-speech-writer`'s `JudgeProfilesPanel` predicate exactly.
  ...
  bulk-importing a scouted round in one tab now shows up in every other open
  tab without a manual reload. The in-progress "Log a scouted round" form
  draft, the built "Compare vs. opponent" comparison, and the bulk-import
  textarea are left untouched, only the persisted roster/history re-reads,
  matching every other closed panel's "refresh the derived view, not the
  draft" convention.
  ...
  though its `form` is a live, directly-editable settings form rather than a
  ...
  during the RSC server-bundle scan every prior run in this tracker has
  documented — a sandbox/toolchain limitation unrelated to this change, not
