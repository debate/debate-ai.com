...
- **🕵️ Opponent Team Profiles — cross-tab live update.** Another repeat of
  ...
  for `OpponentTeamProfilesPanel` — explicitly named as the next open item
  by the previous run's own follow-up note, confirmed still missing a
  `storage`-event listener by grepping every panel in `debate-round`,
  `debate-practice-drills`, and `debate-team-collaboration` for
  ...
  the panel's backing stores (`debate-data-sync`'s `opponentTeamProfiles`,
  the aggregated roster; `opponentRoundRecords`, the logged-round history;
  and `opponentRoundRecordEditHistory`/`opponentRoundRecordRedoHistory`,
  which decide whether a round shows an Undo/Redo action) — mirroring
  `debate-speech-writer`'s `JudgeProfilesPanel` predicate exactly, one
  package over. `OpponentTeamProfilesPanel.tsx` now subscribes to
  `window`'s `storage` event and calls its existing `refresh()` closure
  when the predicate matches — a teammate logging, editing, undoing/
  redoing, deleting, or bulk-importing a scouted round for a team in one
  tab now shows up in every other open tab without a manual reload. The
  in-progress "Log a scouted round" form draft, "Compare vs. opponent"
  selection, and "Bulk import (CSV)" textarea are left untouched, matching
  ...
  `JudgeProfilesPanel` cases). `UserSettingsPanel` (`debate-round` — its
  ...
  values, not stomping an unsaved in-progress edit), `WordLimitPresetsPanel`
  and `DebateRoundPanel` (`debate-round`), `CoachingProgramsPanel`,
  `AccountNotificationsPanel`, `ContactsPanel`, and `SharedCardsPanel`
  (`debate-team-collaboration`), and every panel in `debate-practice-drills`
