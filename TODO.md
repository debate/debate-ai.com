...
### Completed
- **Coaching Programs — wire Practice Round Simulator setup/feedback into the board.**
  Closes idea #13's ("Coaching Programs and Group Challenges") remaining
  follow-up (c): "wiring a member's practice-round setup/feedback (Practice
  Round Simulator) into the space." No follow-ups remain open on idea #13.
  `round/coaching-program.ts` gets a `CoachingProgramMemberPracticeRound`
  type (`contributorId`, `roundId`, `setup`, `feedback?`) and a
  `memberPracticeRounds` field on both `BuildCoachingProgramBoardInput` and
  `CoachingProgramBoard`, populated and roster-filtered the same way as the
  existing `memberFlows`/`memberDrills` pair, plus a
  `buildMemberPracticeRoundSummaryText` renderer reusing
  `practice-round-simulator.ts`'s existing `buildPracticeRoundSetupText`/
  `buildPracticeRoundFeedbackText`. `state/roundContributorFlows.ts` adds
  `buildCoachingProgramMemberPracticeRounds(memberIds)`, reusing its
  existing `roundId`-to-contributor mapping (the same one
  `buildCoachingProgramMemberFlows` reads) to look up each roster member's
  `roundId` against `state/practiceRounds.ts`'s `getPracticeRound` — a
  member is included only when both a recorded flow and a persisted
  Practice Round Simulator round exist for the same `roundId`.
  `state/persistedCoachingProgramBoard.ts`'s `buildPersistedCoachingProgramBoard`
  defaults `memberPracticeRounds` from that resolver (still overridable by
  an explicit argument, mirroring `memberFlows`). `CoachingProgramsPanel.tsx`
  gets a read-only "Member practice rounds" section under "Member flows",
  showing each qualifying member's judge-paradigm and AI-opponent-persona
  badges plus a "Feedback ready"/"No feedback yet" badge, linking to
  `/practice-round` to configure or generate one — no new panel state or
  actions, since the section is driven entirely by the already-loaded board.
  Also corrects a stale claim in `docs/features/practice-round-simulator.md`'s
  "Known gaps" (it said AI speech-generation/judge-decision calls were still
  open; they've since shipped) and documents the real remaining gap there:
  nothing calls `buildPracticeRoundFeedback` to ever populate
  `PracticeRoundRecord.feedback`, tracked as a new follow-up (d) on the
  "Practice Round Simulator" bullet in TODO.md's Research Crowdsourcing
  Organizer Features list — not started. Vitest-covered in
  `packages/debate-round/test/coaching-program.test.ts`,
  `test/roundContributorFlows.test.ts`, and
  `test/persistedCoachingProgramBoard.test.ts` (roster-filtered resolution,
  missing-practice-round exclusion, no-flow exclusion, summary-text
  rendering, and default-vs-override board composition).
