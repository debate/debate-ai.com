...
### Completed
- **Daily Best Card Challenge — submittedAt persistence + banner/widget UI.**
  `packages/debate-card-search/src/lib/contribution-leaderboard.ts`'s
  `AttributedContribution` gains an optional `submittedAt` field, stamped by
  `panels/ContributionsFeedPanel.tsx`'s submit handler. `state/contributions.ts`
  adds `buildPersistedDailyBestCards`/`getPersistedBestCardForDay`, composing
  the already-built `lib/daily-best-card.ts` pure day-grouping/winner-picking
  logic against the persisted store's timestamped, `kind: "card"`
  contributions (a private `readTimestampedCardContributions` helper narrows
  and excludes untimestamped/non-card records). A new
  `packages/debate-card-search/src/panels/DailyBestCardPanel.tsx` renders
  today's winning card as a highlight banner plus a history of prior days'
  winners, mounted at `/cards/daily-best` — closing follow-ups (a) and (c)
  under the "🕵️ Daily Best Card Challenge" bullet. Follow-up (b), a scheduled
  job to persist/announce a day's winner on a cadence, remains open — no
  scheduler exists in this repo. Vitest-covered in
  `packages/debate-card-search/test/contributions.test.ts` (new
  `buildPersistedDailyBestCards`/`getPersistedBestCardForDay` describe blocks:
  empty-store, untimestamped-excluded, non-card-excluded, and multi-day
  winner-selection cases). Verified: `bun install` (2050 packages), `bun run
  typecheck` (12 packages, all pass), `bun run test` (93 files / 1339 tests,
  all pass), and `bun run build:web` (`debate-ai-web`, succeeds,
  `/cards/daily-best` route present) all pass. No repo-wide `lint` script
  exists to run. Documented in `docs/features/daily-best-card-challenge.md`.
