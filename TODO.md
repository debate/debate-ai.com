...
### Completed
- **Coaching Programs and Group Challenges — wire a member's Practice Round
  Simulator setup/feedback into the coaching space.** Closes follow-up (c)
  named under idea #13 ("Coaching Programs and Group Challenges") in
  TODO.md's Product Feature Ideas list: "wiring a member's practice-round
  setup/feedback (Practice Round Simulator) into the space." `debate-round`
  adds `state/coachingProgramMemberRounds.ts` — a `MemberRoundLink`
  (`programId`/`memberId`/`roundId`) localStorage store, mirroring the
  existing `coachingPrograms.ts` persistence convention, with
  `listMemberRoundLinks`/`getMemberRoundLink`/`saveMemberRoundLink`
  (upsert)/`deleteMemberRoundLink`/`deleteMemberRoundLinksForProgram` — plus
  `round/coaching-program-member-round-wiring.ts`'s
  `buildCoachingProgramMemberRoundStatuses` (resolves every roster member's
  linked `roundId` through the already-persisted `state/practiceRounds.ts`,
  mirroring `pre-round-briefing.ts`'s `buildPreRoundBriefingFromStores`
  store-composition convention) and `buildMemberPracticeRoundStatusText`
  (renders through `practice-round-simulator.ts`'s own
  `buildPracticeRoundSetupText`/`buildPracticeRoundFeedbackText` rather than
  introducing a second rendering). `CoachingProgramsPanel` now renders each
  roster member with a practice-round-ID Link/Update/Unlink control and that
  member's resolved setup/feedback status inline, and removing a program now
  also clears its members' links via `deleteMemberRoundLinksForProgram`. No
  new practice-round or coaching-program-board logic is introduced. Docs
  added at `docs/features/coaching-programs.md`. Vitest-covered in
  `packages/debate-round/test/coachingProgramMemberRounds.test.ts` (store
  CRUD, program-scoping, upsert, bulk-delete-by-program, corrupt/missing
  JSON) and
  `packages/debate-round/test/coaching-program-member-round-wiring.test.ts`
  (status resolution across unlinked/linked-and-found/linked-but-cleared/
  program-scoped-lookup cases, and the rendered status text for each).
  Follow-up "(b-continued)" — a dashboard view rendering each program's full
  `buildCoachingProgramBoard` — remains open; it still needs persisted
  challenge win events and live topic-sprint contributions in a form the
  board could read directly, neither of which this slice adds. `bun run
  typecheck` (11 packages, all pass), `bun run test` (144 files / 1957
  tests, all pass), and `bun run build` all pass. No repo-wide `lint` script
  exists (checked root/package `package.json` scripts), matching this
  bullet's prior entries. PR:
  [#234](https://github.com/debate/debate-ai.com/pull/234).
...
13. **Coaching Programs and Group Challenges** — Enable coaches to create group coaching spaces with assigned drills, research sprints, practice rounds, shared feedback, progress tracking, and friendly challenges such as completing a set of blocks or winning a rebuttal exercise. _Status: first slices done (see Tracker Status above) — the "friendly challenges" half has `debate-card-search`'s `group-challenges.ts` (`buildGroupChallengeBoard`), and the coaching-space model tying it together has `debate-round`'s `coaching-program.ts` (`buildCoachingProgramBoard`), composing that group-challenge board with the existing Team Collaboration Mode topic sprint and AI Drill Generator drill sets per roster member. A second slice, `coachingPrograms.ts` (see Tracker Status above, "Coaching Program Persistence — localStorage config store"), now persists a `CoachingProgramConfig` to localStorage, closing follow-up (a). A third slice, `CoachingProgramsPanel` (see Tracker Status above, "Coaching Programs and Group Challenges — coaching-program config UI"), now renders a create-program form and every persisted program's roster at `/coaching-programs`, closing the config-management half of follow-up (b). A fourth slice, `GroupChallengesPanel` (see Tracker Status above, "Group Challenges — challenge-board/creation UI"), now renders a create-challenge form and every persisted `GroupChallenge` at `/cards/group-challenges`, closing the "Group Challenge Persistence" entry's follow-up (a). A fifth slice (see Tracker Status above, "Coaching Programs and Group Challenges — wire a member's Practice Round Simulator setup/feedback into the coaching space") added `state/coachingProgramMemberRounds.ts` (a persisted `MemberRoundLink` roundId-to-member mapping, scoped per program) and `round/coaching-program-member-round-wiring.ts`'s `buildCoachingProgramMemberRoundStatuses`, wiring a Link/Update/Unlink control per roster member into `CoachingProgramsPanel` that resolves and renders that member's linked Practice Round Simulator setup/feedback inline, closing follow-up (c). Follow-up (b-continued), a dashboard view that renders each program's full `buildCoachingProgramBoard`, remains open — it still needs persisted challenge win events and live topic-sprint contributions in a form the board could read directly; the new roundId-to-member mapping resolves a member's practice-round *status* for display, but `buildCoachingProgramBoard`'s `memberFlows` input needs each member's actual flowed `Flow` object, which no store yet persists. Not started._
...
* 🏆 Top Contributor Awards - Give recognition for best evidence finder, best explainers, best original argument, and best refutations. _Status: first slices done (see Tracker Status above) — `debate-card-search` now has `buildTopContributorAwards`/`buildCategoryLeaderboard`/`groupContributionsByKind`/`buildAwardsAnnouncementText` for grouping contributor-attributed contributions by `ContributionKind` and selecting a per-kind category winner by helpfulness score, reusing the existing idea #11/Contribution Leaderboard scoring. A second slice, `ContributorAwardsPanel` (see Tracker Status above, "Top Contributor Awards — awards UI panel"), now renders every category's current winner at `/cards/awards`, closing follow-up (c). A third slice, `state/contributorAwardAnnouncements.ts` (see Tracker Status above, "Top Contributor Awards — announce/freeze action"), now lets the current day's standings be frozen via an "Announce today's awards" action (mirroring the Daily Best Card Challenge's identical announce pattern) and renders the announced history, closing follow-up (b). Follow-up (a), a finer-grained kind/tag for "original argument" and "refutation" contributions, neither of which exists as a distinct kind today, remains open — not started._
