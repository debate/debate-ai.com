...
const main = () => {
  if (process.argv.length !== 3) {
    console.log("Usage: node extract_cards.js <FOLDER>");
    return;
  }
  ...
  allFiles.forEach(function(inputFilePath) {
    ...
    var outputPath=inputFilePath.replace(".html", ".json")
    // outputPath = outputPath.replace(INPUT_FOLDER.replace(/\\+$/g, ''), INPUT_FOLDER.replace(/\\+$/g, '') + " JSON")
        ...
        if (isHeading && isLink) {
          if (cardLink !== "") {
            ...
            if (cleanedCardHtml !== "") {
              ...
              cards.push([cleanedCardHtml, cardLink]);
            }
          }
          cardHtml = "";
          cardLink = "";
