<script lang="ts">
  ...
  import audioDataBeep from './final-beep.js';
  ...
  function displayEndTime() {
    if (typeof window !== "undefined") {
      ...
      // play final beep sound - bang from movie "Inception"
      new Audio(audioDataBeep).play();

      // (new Audio(audioDataBeep './assets/final_beep.mp3')).play();
  ...
  function fixTime(time: string) {
    ...
    let timeString = Math.min(
      59,
      Math.max(0, timeNum != timeNum ? 0 : timeNum)
    ).toString();
...
<div
  class="time"
  class:invalid
  class:done={time == 0 && state.name == "running"}
>
  <input
    type="text"
    bind:value={minutes}
    ...
    style={"--chars: " +
      (minutes == null ? 1 : minutes?.toString()?.length ?? 1).toString()}
    class="minutes"
  ...
  <input
    ...
    style={"--chars: " +
      (seconds == null ? 1 : seconds?.toString()?.length ?? 1).toString()}
    class="seconds"
    on:focus={selectOnFocus}
