<script lang="ts">
	...
	import { activeMouse, flows, flowsChange, selected } from './models/store';
	import { boxFromPath, newBox } from './models/flow';
	import { isMergingFlows } from './models/sharing';
	import type { Box, Flow } from './models/type';
	import { createKeyDownHandler, type KeyComboOptionsIndex } from './models/key';
		...
		commandControl: {
			...
			Enter: {
				handle: () => {
					...
					blurSelf();
					focusAdjacent(1) ||  addSibling(0, 1) && focusSibling(0, 1);
				}
			}
		},
		alt: {
