<script lang="ts">
  ...
  function handleViewModeChange(selectedValue: string) {
    ...
    dispatch('viewModeChange', { value: viewMode });
  }
...
<div class="border-b p-2 flex items-center space-x-4 shadow-md bg-white">
  <Select.Root onSelectedChange={handleViewModeChange}>
        ...
        {#each recentBlocks as block (block.blockIndex)}
          {#if block && block.blockIndex !== undefined}
            <Tabs.Trigger
              ...
              on:click={() => handleRecentBlockClick(block)}
            >
