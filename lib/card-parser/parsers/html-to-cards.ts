...
import { FORMAT_PROFILES } from "../utils/format-profiles";
import { extractCiteInfo, cleanUrl } from "../extractors/citation-extractor";
import { finalizeCard, repairCards } from "../utils/card-utils";
import { parseFileNameParts } from "../extractors/file-name-parser";
import type {
  Card,
  FormatProfile,
  MutableCard,
...
export function htmlToCards(
  ...
  let cardState: "OUTSIDE_CARD" | "SEEN_SUMMARY_LINE" | "IN_CARD" =
    "OUTSIDE_CARD";
  ...
  const parser = new Parser(
    {
      ...
      ontext(text: string) {
        ...
        if (currentCard && isInParagraph)
          currentCard.htmlBuffer = (currentCard.htmlBuffer ?? "") + text;
      },
  ...
  return {
    metadata: {
      ...
      blocks: outline.filter((item) => !isCardNode(item) && item.type === 3)
        .length,
    },
