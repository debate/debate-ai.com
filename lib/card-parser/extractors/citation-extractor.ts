...
import { extractHumanName } from "../human-name/human-name-recognizer";
import type { CitationInfo } from "../types/types";
 ...
 */
export function extractCiteInfo(
  fullCite: string,
  boldText: string,
): CitationInfo {
  ...
  if (!info.author && boldText) {
    ...
    if (!authorExtracted) {
      ...
      if (humanNameResult?.author_cite) {
        info.author =
          humanNameResult.author_short || humanNameResult.author_cite;
      ...
      if (boldText.trim() === "'" || boldText.trim() === "''") {
        ...
        const nameBeforeQuote = fullCite.match(
          /([A-Za-z][A-Za-z\s,.-]+?)\s*['"]/,
        );
        if (nameBeforeQuote) {
          ...
          if (nameParts.length > 0)
            info.author = cleanPunctuation(nameParts[nameParts.length - 1]);
        }
        ...
        if (authorMatch) {
          ...
          if (humanNameResult?.author_cite) {
            ...
            const nameParts = authorMatch
              .split(/\s+/)
              .filter((part) => part.length > 0);
            if (nameParts.length > 0) {
              const lastName = cleanPunctuation(
  ...
  if (info.author) {
    ...
    const hasCommonShortWords = authorWords.some((word) =>
      COMMON_SHORT_WORDS.includes(word.toLowerCase()),
    );
  ...
  if (
    info.author &&
    (info.author.trim().length <= 1 ||
      /^['".,;:!?()]+$/.test(info.author.trim()))
  ) {
    info.author = null;
  }
 ...
 */
export function recalculateAuthor(
  fullCite: string,
  _boldText: string,
): string | null {
  ...
  for (const pattern of bracketPatterns) {
    ...
    for (const match of matches) {
      ...
      if (nameParts.length >= 1 && nameParts.length <= 3) {
        const hasCommonWords = nameParts.some((part) =>
          COMMON_SHORT_WORDS.includes(part.toLowerCase()),
        );
